-- Security hardening migration (constraint guard by name-only)
-- Ensure unique model per board only if constraint name not present anywhere
DO $$ BEGIN
  IF NOT EXISTS (
...
-- Apply RLS and policies after constraint
