-- Stage 5 Migration: Global Filters, Alerts, Cache, RBAC, Scheduler
...
create policy if not exists "prefs owner" on public.user_prefs
...
create policy if not exists "alerts owner" on public.alert_rules
for all to authenticated using (created_by = auth.uid()) with check (created_by = auth.uid());
create policy if not exists "events owner" on public.alert_events
...
create policy if not exists "cache owner" on public.aggregate_cache
...
create policy if not exists "role self select" on public.user_roles
for select to authenticated using (user_id = auth.uid());
create policy if not exists "role self upsert" on public.user_roles
for insert to authenticated with check (user_id = auth.uid());
create policy if not exists "role self update" on public.user_roles
