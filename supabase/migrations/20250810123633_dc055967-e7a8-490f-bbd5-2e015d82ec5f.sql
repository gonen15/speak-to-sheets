...
create policy if not exists "ud select" on public.uploaded_datasets
  for select to authenticated
  using (created_by = auth.uid());
create policy if not exists "ud insert" on public.uploaded_datasets
...
create policy if not exists "dr select" on public.dataset_rows
  for select to authenticated
  using (
    exists (
      select 1
...
create policy if not exists "dr insert" on public.dataset_rows
  for insert to authenticated
  with check (
    exists (
      select 1
