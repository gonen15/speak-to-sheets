...
drop policy if exists "dashboards sel (owner)" on public.user_dashboards;
drop policy if exists "dashboards ins (owner)" on public.user_dashboards;
drop policy if exists "dashboards upd (owner)" on public.user_dashboards;
create policy "dashboards sel (owner)"
  on public.user_dashboards for select to authenticated
  using (created_by = auth.uid());
create policy "dashboards ins (owner)"
  on public.user_dashboards for insert to authenticated
  with check (created_by = auth.uid());
create policy "dashboards upd (owner)"
...
drop policy if exists "widgets sel (owner)" on public.dashboard_widgets;
drop policy if exists "widgets ins (owner)" on public.dashboard_widgets;
drop policy if exists "widgets upd (owner)" on public.dashboard_widgets;
create policy "widgets sel (owner)"
...
create policy "widgets ins (owner)"
...
create policy "widgets upd (owner)"
...
drop policy if exists "alerts sel (owner)" on public.alert_rules;
drop policy if exists "alerts ins (owner)" on public.alert_rules;
drop policy if exists "alerts upd (owner)" on public.alert_rules;
create policy "alerts sel (owner)"
  on public.alert_rules for select to authenticated
  using (created_by = auth.uid());
create policy "alerts ins (owner)"
  on public.alert_rules for insert to authenticated
  with check (created_by = auth.uid());
create policy "alerts upd (owner)"
...
drop policy if exists "events sel (owner)" on public.alert_events;
drop policy if exists "events ins (owner)" on public.alert_events;
create policy "events sel (owner)"
...
create policy "events ins (owner)"
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'semantic_models_board_unique'
  ) THEN
